...
class IeeeR10LinkedinScraper(BaseScraper):
    @property
    def source_name(self) -> str:
        return "ieee_r10_linkedin"

    @property
    def start_url(self) -> str:
        return "https://sg.linkedin.com/company/ieee-region10"
            ...
            try:
                ...
                for item in graph:
                    if item.get("@type") == "DiscussionForumPosting":
                        ...
                        results.append({
                            "title": f"IEEE R10 Update: {clean_title}",
                ...
                logger.error(f"R10 LinkedIn JSON-LD parsing exception: {e}")
    ...
    def _parse_dom(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        for overlay in overlay_links:
            ...
            for link in card.find_all("a", href=True):
                href = link["href"]
                if any(x in href.lower() for x in ["company/ieee-region10", "cold-join", "login", "signup", "social-actions"]):
                    continue
                if any(domain in href.lower() for domain in ["bit.ly", "lnkd.in", "youtube.com", "youtu.be", "ieee-region10", "ieee.org"]):
                    ext_url = href
                    break
    ...
    async def parse(self, response: httpx.Response) -> List[Dict[str, Any]]:
        ...
        if not results:
            logger.info("R10 LinkedIn: No posts in JSON-LD. Falling back to HTML DOM selectors.")
            results = self._parse_dom(soup)
            
        logger.info(f"R10 LinkedIn Scraper parsed {len(results)} updates successfully.")
        return results
